...
アプリの起動時に、入力データを指定する。
入力データの指定方法は、以下の2種類がある。
入力ファイルが指定された場合、標準入力は無視する。

- コマンドラインで入力ファイルのパスを指定。
- 標準入力に与える。
...
この後のアプリ本体のループ処理では、
プロンプト表示とユーザ入力の受付けを繰り返す。
プロンプト表示前に標準入力に残っているゴミを拾わないように、
アプリ起動後に一回入力バッファを読み飛ばしておく。

#### アプリの起動 (標準入力指定)

標準入力を入力データとする場合、
prev_contents は空の文字列を設定する。
...
標準入力を使用する場合、ファイルの全体が読み込めなかった場合でも、
分割データのサイズまで読み込めた時点で、エンコードを判定する。

また、curr_index に 0 を設定する。

#### アプリの本体のループ処理

入力データの curr_index から、
...
quitの場合
- clipboardをクリアする。
- 次の処理に進むことなく、アプリケーション全体を終了する。

#### ファイナライズ処理

ファイナライズ処理は、
入力データを入力ファイルとして指定され場合にのみ実施する。
入力データを標準入力として指定され場合は、終了処理に進む。
